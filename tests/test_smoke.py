...
def pipeline():
    ...
    has_docs = any(corpus_dir.glob(pat) for pat in ("*.pdf", "*.md", "*.txt")) if corpus_dir.exists() else False
    if not has_docs:
        pytest.skip("data/corpus/ vazio — adicione pelo menos 1 documento (.pdf, .md ou .txt)")
